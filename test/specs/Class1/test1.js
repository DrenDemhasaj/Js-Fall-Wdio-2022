...
describe("Sample Testcases", () => {
  ...
  it("Verify facebook url", async () => {
    ...
    expect(currentUrl, "facebook.com does not to go expected url").to.equal(
      expectedUrl
    );
  });
  ...
  it("Verify facebook title", async () => {
    ...
    expect(
      currentTitle,
      "facebook.com does not have the expected title"
  ...
  it('Verify facebook title starts with "faCeBooK" (ignore cases)', async () => {
    ...
    const isTitleStartsAsExpected = currentTitle
      .toLowerCase()
      .startsWith(expectedStartsWith.toLowerCase());
    expect(
      isTitleStartsAsExpected,
      'Facebook title does not start with "Facebook"'
    ).to.be.true;
  });
});
