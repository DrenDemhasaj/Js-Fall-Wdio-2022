 ...
 * output: [-1, 2, 43, 65]
 */
 function removeNumFromArray2(numToRemove, numArray) {
    return numArray.filter((num) => (num != numToRemove))
  }
  function removeNumFromArray(num, arr) {
    let i = 0;
...
const numArrayAfterRemoval1_1 = removeNumFromArray2(2, arr1_1);
...
function convertingStringIntoTitleCase(inputString){
...
function titleCaseAllStudentNames(allStudentData) {
    allStudentData.forEach(student =>{
        student.name = convertingStringIntoTitleCase(student.name)
    })
...
function titleCaseAllStudentNames2(allStudentData){
    allStudentData.map(student=> convertingStringIntoTitleCase(student.name))
    return allStudentData
}

function titleCaseAllStudentNames3(allStudentData){
    allStudentData.forEach(student=>{
        student.name = student.name.toLowerCase().split(' ').map(W=>
